...
records = LOAD '$input' AS (date:chararray, time:chararray, xedgelocation:chararray, scbytes:chararray, cip:chararray, csmethod:chararray, csHost:chararray, csuristem:chararray, scstatus:chararray,csReferer:chararray, csUserAgent:chararray, csuriquery:chararray,csCookie:chararray, xedgeresulttype:chararray, xedgerequestid:chararray);
named = FOREACH records GENERATE getNames($7) AS name, getHit($13) AS hit, getError($13) AS error;
grouped = GROUP named BY $0;
results = FOREACH grouped GENERATE group, ratio(SUM($1.hit), COUNT($1.hit)), ratio(SUM($1.error), COUNT($1.error)), GetYear(CurrentTime()), GetMonth(CurrentTime()), GetDay(CurrentTime()), GetHour(CurrentTime());
STORE results into '$output/2017-02-12';
