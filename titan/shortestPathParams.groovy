...
conf.setProperty("storage.hbase.table", "linksSample")
graph = TitanFactory.open(conf);
t=graph.traversal();
while(true){
	println "first input";
	String firstvalue=System.in.newReader().readLine();
	if(firstvalue=="quit"){
		System.exit(0);
	}
	println "second input";
	String secondvalue=System.in.newReader().readLine();
	if(secondvalue=="quit"){
		System.exit(0);
	}
	def x = t.V().has("pageID", firstvalue).repeat(out().simplePath()).until(has('pageID',secondvalue)).path().by('pageID').limit(1).next();
	println x;
}
